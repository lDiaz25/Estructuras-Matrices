...
int  ganador (char c[3][3]);        // 0 = gana jugador, 1 = gana pc, 2 = empate, -1 = sigue

// extras para menú y marcador
int  menu();
...
int main(void) {
    ...
    srand((unsigned)time(NULL));     // semilla una vez al inicio
...
int menu() {
    char buf[32];
    system("clear"); // en Windows usar cls
...
int partida(char c[3][3]) {
    ...
    do {
        ...
        // desde la quinta jugada ya puede haber ganador
        if (turnos >= 4) {
            g = ganador(c);
            if (g != -1) {
                system("clear");
                ...
                // opción para volver a jugar o regresar al menú
                while (1) {
                    ...
                    if (buffer[0] == '1' && buffer[1] == '\n') {
                        return partida(c); // reinicia tablero y repite la partida
                    }
                    if (buffer[0] == '2' && buffer[1] == '\n') {
                        return g;          // regresa al menú
                    }
...
// inicializa tablero con valores 1..9
void Intro_Primera (char c[3][3]) {
    int i, j;
    char aux = '1';
    for(i=0; i<3; i++) {
...
// pide movimiento al jugador (solo acepta un número del 1 al 9)
void Intro_Yo (char c[3][3]) {
    int i, j, k;
    char aux;
    char buffer[100];
    ...
    do {
        do {
            ...
            // verifico que sea un solo dígito válido
...
// comprueba si alguien gana o hay empate
int ganador (char c[3][3]) {
    ...
    // si aun hay casillas libres, sigue el juego
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
            if (c[i][j] != 'X' && c[i][j] != 'O')
                return -1;
